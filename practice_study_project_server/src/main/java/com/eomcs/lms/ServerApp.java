...
public class ServerApp {
  ...
  public static void main(String[] args) {
    ...
    /*
     * try ( // 서버쪽 연결 준비 // => 클라이언트의 연결을 9999번 포트에서 기다린다. 
       ServerSocket serverSocket = new ServerSocket(9999);) {
     *
     * System.out.println("클라이언트 연결 대기중...");
     *
     * while (true) { // 서버에 대기하고 있는 클라이언트와 연결 // => 대기하고 있는 클라이언트와 연결될 때까지 리턴하지 않는다. 
     * Socket socket = serverSocket.accept(); System.out.println("클라이언트와 연결되었음!");
     *
     * // 클라이언트의 요청 처리 
     * 
  ...
  public void service() {
    ...
    // 옵저버의 실행이 끝났으면 DataLoaderListener 옵저버가 준비한 List 객체를 꺼내보자!
  ...
  static void processRequest(Socket clientSocket) {
    ...
    try (
        ...
        // 클라이언트의 메시지를 수신하고 클라이언트로 전송할 입출력 도구 준지
